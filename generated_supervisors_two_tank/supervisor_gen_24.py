def supervise(telemetry_window, active_setpoints, nominal_targets):
    ...
    spike1 = (te1 > 2.8) and (ae1 > 0.20)
    abs1 = (te1 > 1.8) and (ae1 > 0.20)
    dev1_hi = (dev1 > 0.35) and (ae1 > 0.15)
    sig1 = frac_rel(eff1, err1, ref1, 0.35, 0.12, w)
    sust1 = sig1 >= 0.45
    raw1 = spike1 or abs1 or dev1_hi or sust1

    spike2 = (te2 > 2.5) and (ae2 > 0.25)
    abs2 = (te2 > 1.6) and (ae2 > 0.18)
    dev2_hi = (dev2 > 0.30) and (ae2 > 0.14)
    sig2 = frac_rel(eff2, err2, ref2, 0.35, 0.12, w)
    sust2 = sig2 >= 0.45
    raw2 = spike2 or abs2 or dev2_hi or sust2

    t2_long = frac_abs(eff2, err2, 1.6, 0.18, wlong) > 0.5
    ...
    if h > k:
        h = k
    rec1 = (k >= 4) and (mean(tail(eff1, h)) <= min(ref1 + 0.45, 2.0)) and (mean_abs(tail(err1, h)) <= 0.15)
    rec2 = (k >= 4) and (mean(tail(eff2, h)) <= min(ref2 + 0.35, 1.7)) and (mean_abs(tail(err2, h)) <= 0.14)

    tank1_anom = bool(raw1 and not rec1)

    upstream1 = bool(raw1 or (dev1 > 0.30) or (te1 > 1.8))
    ...
    elif (dev1 < 0.25) and (ae1 < 0.15) and (t1_sp < nom1):
