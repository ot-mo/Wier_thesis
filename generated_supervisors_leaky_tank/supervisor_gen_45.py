def supervise(telemetry_window, active_setpoint, nominal_target):
    ...
    persistent_severe = (
        short_n >= 3 and
        med_dev_short > severe_threshold and
        med_error_short < -0.05
    )

    error_severe = (
        short_n >= 2 and
        med_error_short < -0.08 and
    ...
    standard_short = (
        ...
        short_exceed_mod >= max(2, int(short_n * 0.7)) and
        med_error_short < -0.015 and
        mean_dev_short > moderate_threshold * 0.6
    )
    ...
    standard_long = (
        ...
        long_exceed_mild >= max(4, int(long_n * 0.7)) and
        med_error_long < -0.015
    )
    ...
    mild_long = (
        ...
        long_exceed_mild >= max(5, int(long_n * 0.6)) and
        robust_sigma < 0.25 and
        med_error_long < -0.01
    )

    anomaly_flag = fast_severe or persistent_severe or error_severe or error_persistent or standard_short or standard_long or mild_long
    ...
    clear_long = (
        ...
        long_exceed_mild <= int(long_n * 0.2)
    )

    if anomaly_flag and (clear_short or clear_long):
        anomaly_flag = False
    ...
    if anomaly_flag:
        ...
        if (active_setpoint < nominal_target and
            clear_short and clear_long and
            med_dev_short < 0.04 and
            ...
            diagnosis = "System stable and below nominal, restoring setpoint slowly."
        else:
