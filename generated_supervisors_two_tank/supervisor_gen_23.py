def supervise(telemetry_window, active_setpoints, nominal_targets):
    ...
    spike1 = (te1 > 3.0) and (ae1 > 0.20)
    abs1 = (te1 > 2.3) and (ae1 > 0.18)
    dev1_hi = (dev1 > 0.35) and (ae1 > 0.14)
    sig1 = frac_rel(eff1, err1, ref1, 0.30, 0.12, w)
    sust1 = sig1 >= 0.5
    raw1 = spike1 or abs1 or dev1_hi or sust1

    spike2 = (te2 > 2.6) and (ae2 > 0.24)
    abs2 = (te2 > 1.8) and (ae2 > 0.18)
    dev2_hi = (dev2 > 0.30) and (ae2 > 0.12)
    sig2 = frac_rel(eff2, err2, ref2, 0.25, 0.10, w)
    sust2 = sig2 >= 0.5
    raw2 = spike2 or abs2 or dev2_hi or sust2

    t2_long = frac_abs(eff2, err2, 1.9, 0.18, wlong) > 0.6

    upstream1 = raw1 or (dev1 > 0.25) or (te1 > 2.2)
    cascade = bool(upstream1 and raw2 and (not spike2) and (not t2_long) and (dev2 < 0.30))
    ...
    if h > k:
        h = k
    rec1 = (k >= 4) and (mean(tail(eff1, h)) <= ref1 + 0.40) and (mean_abs(tail(err1, h)) <= 0.14)
    ...
    elif (dev1 < 0.25) and (ae1 < 0.14) and (t1_sp < nom1):
