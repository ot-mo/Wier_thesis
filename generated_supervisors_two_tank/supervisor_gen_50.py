def supervise(telemetry_window, active_setpoints, nominal_targets):
    ...
    def theil_sen_early_residuals(xs, ys, early_n):
        m = len(xs)
        fit_n = min(m, early_n)
        if fit_n < 3:
            return None
        ...
        if not slopes:
            return None
        slope = median(slopes)
        intercepts = [ys[i] - slope * xs[i] for i in range(fit_n)]
    ...
    abs_eff1 = te1 > 2.35
    spike1 = te1 > 3.00
    dev1_anom = (dev1 > 0.45 and ae1 > 0.06) or (dev1 > 0.75)
    sustained_thresh1 = max(ref1 + 0.40, 2.10)
    ...
    tank2_resid_anom = False
    coupl = theil_sen_early_residuals(eff1, eff2, early_n)
    if coupl is not None:
        _, _, coupl_resid2 = coupl
        early_res = coupl_resid2[:early_n]
        base_res = median(early_res)
        mad_res = median([abs(v - base_res) for v in early_res])
        scale_res = 1.4826 * mad_res
        if scale_res < 0.03:
            scale_res = 0.03
        thresh_res = base_res + 2.5 * scale_res
        if thresh_res < 0.20:
            thresh_res = 0.20
        recent_res = tail(coupl_resid2, k)
        med_res = median(recent_res)
        frac_res = frac_eff_only(coupl_resid2, thresh_res, w)
        tank2_resid_anom = (frac_res >= 0.5) and (med_res > base_res + 1.0 * scale_res)
    ...
    tank1_anom = bool((raw1 and not rec1) or depressed1)
    tank2_anom = bool((raw2 and not rec2 and not cascade) or depressed2)
    ...
    if tank1_anom:
        ...
        elif depressed1 and rec1:
            new1 = min(nom1, t1_sp + RESTORE_STEP)
            diag1 = 'tank1 recovering from suspected anomaly; verifying clearance at nominal setpoint'
        else:
            new1 = t1_sp
            diag1 = 'tank1 anomaly suspected (setpoint depressed; waiting for stable recovery)'
    ...
    if tank2_anom:
        if raw2 and not rec2 and not cascade:
            cand2 = max(nom2 - MAX_DEPRESS, t2_sp - LOWER_STEP)
            if cand2 > t2_sp:
                cand2 = t2_sp
            new2 = cand2
            diag2 = 'tank2 anomaly suspected (independent, above coupling threshold)'
        elif depressed2 and rec2:
            new2 = min(nom2, t2_sp + RESTORE_STEP)
            diag2 = 'tank2 recovering from suspected anomaly; verifying clearance at nominal setpoint'
        else:
            new2 = t2_sp
            diag2 = 'tank2 anomaly suspected (setpoint depressed; waiting for stable recovery)'
    elif cascade:
        new2 = t2_sp
