def supervise(telemetry_window, active_setpoints, nominal_targets):
    ...
    # Tank 1: slightly relaxed thresholds for earlier detection.
    spike1 = (te1 > 3.2) and (ae1 > 0.20)
    abs1 = (te1 > 2.3) and (ae1 > 0.20)
    dev1_hi = (dev1 > 0.50) and (ae1 > 0.15)
    sig1 = frac_rel(eff1, err1, ref1, 0.40, 0.12, w)
    sust1 = sig1 >= 0.45
    raw1 = spike1 or abs1 or dev1_hi or sust1

    # Tank 2: gravity-fed, lower baseline, so thresholds remain lower than tank1.
    spike2 = (te2 > 2.8) and (ae2 > 0.28)
    abs2 = (te2 > 1.9) and (ae2 > 0.18)
    dev2_hi = (dev2 > 0.40) and (ae2 > 0.12)
    sig2 = frac_rel(eff2, err2, ref2, 0.35, 0.10, w)
    sust2 = sig2 >= 0.45
    raw2 = spike2 or abs2 or dev2_hi or sust2

    # Long organised tank2 signature measured absolutely, used as cascade override.
    t2_long = frac_abs(eff2, err2, 2.0, 0.20, wlong) > 0.55

    # Stricter independent evidence for tank2 when tank1 is also faulted.
    # This reduces cascade false positives while still allowing genuine tank2
    # faults in both_faults to be flagged when observed deviation/effort is high.
    t2_independent = bool(spike2 or t2_long or
                          (te2 > 2.5 and ae2 > 0.25) or
                          (dev2 > 0.70 and ae2 > 0.16))
    ...
    if h > k:
        h = k
    rec1 = (k >= 4) and (mean(tail(eff1, h)) <= ref1 + 0.45) and (mean_abs(tail(err1, h)) <= 0.14)
