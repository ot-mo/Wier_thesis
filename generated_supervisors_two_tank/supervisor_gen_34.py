def supervise(telemetry_window, active_setpoints, nominal_targets):
    ...
    # Low quantile stays near the healthy baseline even when a persistent
    # fault occupies most of the window.
    ref1 = quantile(eff1, 0.05)
    ref2 = quantile(eff2, 0.05)

    dev1 = te1 - ref1
    dev2 = te2 - ref2

    # ---- Tank 1: upstream, larger effort scale ----
    spike1 = (te1 > ref1 + 1.2) and (ae1 > 0.30)
    strong1 = (te1 > ref1 + 0.75) and (ae1 > 0.22)
    dev1_hi = (dev1 > 0.55) and (ae1 > 0.16)
    sust1 = frac_rel(eff1, err1, ref1, 0.45, 0.14, w) >= 0.5
    err1_hi = (ae1 > 0.45) and (dev1 > 0.15)
    raw1 = spike1 or strong1 or dev1_hi or sust1 or err1_hi

    # ---- Tank 2: gravity-fed, lower baseline, softer scale ----
    spike2 = (te2 > ref2 + 1.0) and (ae2 > 0.30)
    strong2 = (te2 > ref2 + 0.65) and (ae2 > 0.22)
    dev2_hi = (dev2 > 0.45) and (ae2 > 0.14)
    sust2 = frac_rel(eff2, err2, ref2, 0.40, 0.12, w) >= 0.5
    err2_hi = (ae2 > 0.40) and (dev2 > 0.12)
    raw2 = spike2 or strong2 or dev2_hi or sust2 or err2_hi

    # Long organized tank2 signature.
    t2_long = (frac_rel(eff2, err2, ref2, 0.55, 0.20, wlong) > 0.6) or (frac_abs(eff2, err2, 1.8, 0.20, wlong) > 0.6)

    # Recovery: only when recent effort is back near the low robust reference.
    h = k // 3
    if h < 3:
        h = 3
    if h > k:
    ...
    # Cascade coupling: tank1 feeds tank2, so an upstream fault can perturb tank2.
    # Only suppress tank2 when its evidence is weak enough to be explained by tank1.
    upstream1 = bool(raw1 or (dev1 > 0.35) or (te1 > ref1 + 0.7))
    t2_independent = bool(
        spike2
        or t2_long
        or (dev2 > 0.65 and ae2 > 0.32)
        or (ae2 > 0.50 and dev2 > 0.15)
    )
    ...
    if tank1_anom:
        ...
        if cand1 > t1_sp:
            cand1 = t1_sp
        new1 = cand1
        diag1 = 'tank1 anomaly suspected (adaptive relative evidence)'
    elif (dev1 < 0.25) and (ae1 < 0.16) and (t1_sp < nom1):
    ...
    if tank2_anom:
        ...
        if cand2 > t2_sp:
            cand2 = t2_sp
        new2 = cand2
        diag2 = 'tank2 anomaly suspected (independent strong evidence)'
    elif cascade:
        new2 = t2_sp
        diag2 = 'tank2 perturbation attributed to upstream tank1 fault (cascade, not flagged)'
    elif (dev2 < 0.25) and (ae2 < 0.14) and (t2_sp < nom2):
